...
\begin{itemize}[leftmargin=*, nosep]
    \sloppy 
    \item \textbf{Propagation Methods (Fast Marching):} These solve the Eikonal equation $|\nabla \phi| = 1$ by propagating a wavefront. While reaching $O(n \log n)$, they are inherently serial (hindering parallelization) and lack reusability—every new source requires a full re-computation.
...
\smallskip
\noindent Our project implements the \textbf{Heat Method}, which overcomes these limitations by pre-factoring the geometry, allowing distance queries to be solved in milliseconds using standard linear algebra.
...
We set the time step to $t = h^2$, where $h$ is the mean edge length, noting that larger steps help bridge artifacts in coarse meshes. We strictly enforce Neumann boundary conditions ($\nabla u \cdot \mathbf{n} = 0$) to prevent heat loss and gradient warping, a property naturally satisfied by the Cotangent Laplacian. Finally, we optimize performance by pre-factoring the symmetric positive-definite system matrices using sparse Cholesky decomposition \cite{crane2013}. While initialization is costly, this allows subsequent queries to be solved via back-substitution, rendering the method orders of magnitude faster than Dijkstra.
...
This shift from scalar to vector diffusion encodes not only how far points lie from the source, but also how directions evolve across curved geometry. The resulting vector field can be used to derive quantities such as the logarithmic map and geometric centers.
...
\textbf{Description.}  
Recovers polar coordinates $(r, \varphi)$ by diffusing a reference vector (for $\varphi$) and a radial vector (for $r$).
...
\begin{figure}[t!]
    ...
    The VHM shows significantly sharper alignment to the reference solution, while older schemes propagate angular errors across the mesh.}
    \label{fig:logmap}
\end{figure}
...
For $p=2$, minimizers are known as Karcher means.
...
\textbf{Description.}
The energy $E_2(x)=\tfrac{1}{2n}\sum_i d(x,y_i)^2$ is minimized by iteratively averaging log vectors. At each iterate $m^k$, the VHM is used to compute $\log_{m^k}(y_i)$ for all data points.
...
\begin{figure}[t!]
    ...
    Methods based on local log-map approximations may fail to converge, whereas the VHM-based log map guides the iterates quickly to the correct symmetric location.}
    \label{fig:karcher}
\end{figure}
...
Table~\ref{tab:method_comparison} presents a detailed comparison between the Scalar Heat Method, Fast Marching (Kimmel \& Sethian), and Exact Polyhedral distance (MMP algorithm).
...
The method is stable with only 1.6\% relative error at $\sigma=0.001$. However, at high noise levels ($\sigma=0.02$), the relative error increases significantly. The noise adds high-frequency spikes that drastically increase the path length between points.
...
In finite element approximations, mesh quality affects solution quality. Empirically, the Heat Method produces \textbf{fewer spurious minima} than Fast Marching or biharmonic distance, due to regularization from the Hodge decomposition step (Step III).
...
Experiments confirm that the time step $t$ acts as a geometric regularizer rather than strictly adhering to $t \to 0$. While larger steps allow heat to bridge discretization artifacts on coarse meshes, excessive $t$ values smooth over genuine curvature. regarding boundaries, Neumann conditions ($\nabla u \cdot \mathbf{n} = 0$) are strictly required to insulate the mesh; in contrast, Dirichlet conditions force artificial heat loss, warping the gradient field near edges.
...
This project highlights the efficiency of shifting from nonlinear hyperbolic formulations to linear elliptic systems. We demonstrated that the Heat Method offers order-of-magnitude speedups via prefactorization and natural regularization, making it superior for general shape analysis.

While the method trades polyhedral precision for robustness, extensions like the Vector Heat Method allow for parallel transport and logarithmic maps. Future research should address linear convergence limitations through adaptive time-stepping and higher-order discretizations to further cement the method's role in geometry processing.
